...
export default function MaleProducts() {
  ...
  const femaleproducts= taskData.filter(product => product.category === 'Female');
  ...
  React.useEffect(() => {
    ...
    const uniqueCategories = [...new Set(femaleproducts.map(product => product.option))];
  ...
  const applyFilters = () => {
    let filteredProducts = [...femaleproducts];

    // Apply category filter
    if (filterState.categoryFilter !== 'All') {
      filteredProducts = filteredProducts.filter(product => product.option === filterState.categoryFilter);
    }
  ...
  return (
    <div className='container-fluid mt-5'>
      ...
      <h2 className='text-center m-5'>Female Products</h2>
